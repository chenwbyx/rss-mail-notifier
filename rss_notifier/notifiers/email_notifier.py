...
class EmailNotifier(Notifier):
    ...
    * ``TO_EMAIL``  - 收件人地址（多个用逗号分隔）
    ...
    def _build_plain_text(articles: NewArticles, total: int) -> str:
        ...
        lines.append("https://github.com/chenwbyx/rss-mail-notifier")
    ...
    def _build_html(articles: NewArticles, total: int) -> str:
        ...
        used_icons: set[int] = set()
        sections: list[str] = []
        for feed_name, feed_articles in articles.items():
            safe_feed_name = html_mod.escape(feed_name)
            idx = zlib.crc32(feed_name.encode()) % len(FEED_ICONS)
            while idx in used_icons:
                idx = (idx + 1) % len(FEED_ICONS)
            used_icons.add(idx)
            icon = FEED_ICONS[idx]
            article_html_parts: list[str] = []
            for article in feed_articles:
                published = html_mod.escape(article.published or "未知")
                safe_title = html_mod.escape(article.title)
                title_html = (
                    f'<span style="font-size:16px;font-weight:600;'
                    f'line-height:1.4;color:#1a1a1a;">{safe_title}</span>'
                )
                link_html = ""
                if article.link:
                    safe_link = html_mod.escape(article.link, quote=True)
                    link_html = (
        ...
        return (
            ...
            f"Generated by "
            f'<a href="https://github.com/chenwbyx/rss-mail-notifier" '
